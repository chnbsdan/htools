...
6. 在 Pages 环境变量中添加后台密码，并将 GitHub 仓库信息读取 Token 配置为 Secret：

```txt
ADMIN_PASSWORD=你的后台密码
GITHUB_TOKEN=你的只读 GitHub Token
```
...
应用会在首次访问 API 时自动初始化 D1 表结构。D1 默认是空的，项目不会在部署时自动写入默认工具数据；你可以在后台导入默认订阅源，也可以手动添加工具。
...
在 `.dev.vars` 中填写本地 `ADMIN_PASSWORD` 和只读 `GITHUB_TOKEN`，并填写 `wrangler.local.toml` 中的 `account_id`、`database_name`、`database_id`，然后启动本地 Pages Functions。`.dev.vars` 已被 Git 忽略，不要把真实 Token 写入 `.dev.vars.example` 或提交到仓库：
...
- 默认工具源访问地址：`https://raw.githubusercontent.com/shaoyouvip/htools/refs/heads/main/public/htools.json`
...
- 请在 Cloudflare Pages 的 Secret 中配置后台密码、`GITHUB_TOKEN` 和 GitHub OAuth Client Secret，不要把它们写入公开代码。
